...
var sendSock = function(data) {
	if(typeof data !== "undefined") {
	    ...
	    logger.output({severity:'debug', source:'Socket | Send',message: data,timestamp: new Date(),location: os.hostname()});
	    ws.send(data);
	    i++;
	} else logger.output({severity:'error', source:'Socket | Send',message:'No arguments.',timestamp: new Date(),location: os.hostname()});
}
var connectSlack = function(wsurl) {
    ws = new webSocket(wsurl);
    ws.on('open', function() {
		logger.output({severity:'info', source:'Socket',message:'Connected as '+slackData.self.name+' ['+slackData.self.id+'].',timestamp: new Date(),location: os.hostname()});
    });
...
// __init__
var slackAPI = function(authtoken) {
	if (!authtoken || typeof authtoken !== 'string' || !authtoken.match(/^([a-z]*)\-([0-9]*)\-([0-9a-zA-Z]*)/)) {
		logger.output({severity:'error', source:'Slack API',message:'Invalid arguments! Please provide a valid auth token.',timestamp: new Date(),location: os.hostname()});
		process.exit(1);
	}
